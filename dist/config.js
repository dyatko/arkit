...
class Config {
    ...
    getUserConfig(options) {
        ...
        const userConfig = (0, utils_1.safeRequire)(userConfigPath);
        const packageJSONPath = path.resolve(this.directory, "package");
        const packageJSON = (0, utils_1.safeRequire)(packageJSONPath);
        if (userConfig) {
            (0, utils_1.debug)(`Found arkit config in ${userConfigPath}`);
            return userConfig;
        }
        if (packageJSON && packageJSON.arkit) {
            (0, utils_1.debug)(`Found arkit config in ${packageJSONPath}`);
            return packageJSON.arkit;
        }
    }
    getFinalComponents(options, userConfig) {
        const userComponents = userConfig && userConfig.components;
        return userComponents ? (0, utils_1.array)(userComponents) : DEFAULT_COMPONENTS;
    }
    getFinalOutputs(options, userConfig) {
        const initialOutputs = (0, utils_1.array)(userConfig && userConfig.output) || [{}];
        ...
        return initialOutputs.map((output) => (Object.assign(Object.assign({}, output), { path: (0, utils_1.array)(output.path || outputOption || "svg"), groups: output.groups || (!userComponents ? generatedGroups : undefined) })));
    }
